...
class ADHDWidget(QWidget):
    def __init__(self):
        super(ADHDWidget, self).__init__()
        self.setLayout(QVBoxLayout())
        att_plot = PlotWidget(title='Attention')
        eye_plot = PlotWidget(title='Eye Movement')
        stable_att_plot = PlotWidget(title='Stable Attention')
        self.att_curve = att_plot.plot(pen=Colors.SINGLE_CURVE)
        self.eye_curve = eye_plot.plot(pen=Colors.SINGLE_CURVE)
        self.stable_att_curve = stable_att_plot.plot(pen=Colors.SINGLE_CURVE)
        for plot in [att_plot, stable_att_plot, eye_plot]:
            plot.setBackground(Colors.PLOT_BACKGROUND)
            plot.showGrid(x=True, y=True, alpha=0.7)
            plot.setYRange(0, 100)
            self.layout().addWidget(plot)

    def update_plot(self, attention=None, eye_movement=None, stable_attention=None):
        for data, curve in zip([attention, eye_movement, stable_attention],
                               [self.att_curve, self.eye_curve, self.stable_att_curve]):
            if data is not None:
                curve.setData(data)
...
class MainWindow(QMainWindow):
    ...
    def __init__(self, app_ctx, parent=None, icon=None, on_tab_change=None):
        ...
        self.adhd_tab = ADHDWidget()
        for tab, title in zip([self.eeg_tab, self.imu_tab, self.ppg_algo_tab, self.ppg_raw_tab, self.adhd_tab],
                              ['EEG', 'IMU', 'PPG-Algo', 'PPG-Raw', 'ADHD']):
        ...
        meditation_plot.addLegend()
        self.meditation_curve = meditation_plot.plot(pen=Colors.DOUBLE_CURVES[1], name='meditation')
        self.awareness_curve = meditation_plot.plot(pen=Colors.DOUBLE_CURVES[0], name='awareness')
        stress_plot = PlotWidget(title='Stress')
        stress_plot.setYRange(0, 100)
    ...
    def update_meditation_plot(self, meditation, awareness):
        if meditation is not None:
            self.meditation_curve.setData(meditation)
        if awareness is not None:
            self.awareness_curve.setData(awareness)
    ...
    def update_adhd_plot(self, attention, eye_movement, weighted):
        self.adhd_tab.update_plot(attention, eye_movement, weighted)
