...
class DeviceListener(QObject, ZenLiteDeviceListener, metaclass=ListenerMetaClass):
    ...
    def __init__(self, gui):
        ...
        self._calmness_buffer = None
        self._awareness_buffer = None
        self._stress_buffer = None
        self._attention_buffer = None
        self._eyemovement_buffer = None
        self._att_weighted_buffer = None
        self._stable_attention_buffer = None
    ...
    def on_attention(self, attention, weighted_attention):
        self.algo_update_signal.emit({"attention": attention, "weighted_attention": weighted_attention})
        if self._attention_buffer is None:
            self._attention_buffer = np.array([attention])
        else:
            self._attention_buffer = trim_data(np.concatenate([self._attention_buffer, [attention]], 0), 0, ALGO_WINDOW_TIME)
        if self._att_weighted_buffer is None:
            self._att_weighted_buffer = np.array([weighted_attention])
        else:
            self._att_weighted_buffer = trim_data(np.concatenate([self._att_weighted_buffer, [weighted_attention]], 0), 0, ALGO_WINDOW_TIME)

    def on_stable_attention(self, stable_attention):
        self.algo_update_signal.emit({"stable_attention": stable_attention})
        if self._stable_attention_buffer is None:
            self._stable_attention_buffer = np.array([stable_attention])
        else:
            self._stable_attention_buffer = trim_data(np.concatenate([self._stable_attention_buffer, [stable_attention]], 0), 0, ALGO_WINDOW_TIME)

    def on_eye_movement(self, eye_movement):
        self.algo_update_signal.emit({"eye_movement": eye_movement})
        if self._eyemovement_buffer is None:
            self._eyemovement_buffer = np.array([eye_movement])
        else:
            self._eyemovement_buffer = trim_data(np.concatenate([self._eyemovement_buffer, [eye_movement]], 0), 0, ALGO_WINDOW_TIME)

    def on_meditation(self, meditation, calmness, awareness):
        self.algo_update_signal.emit({"meditation": meditation})
        self.algo_update_signal.emit({"calmness": calmness})
        self.algo_update_signal.emit({"awareness": awareness})
        ...
        if self._calmness_buffer is None:
            self._calmness_buffer = np.array([calmness])
        else:
            self._calmness_buffer = trim_data(np.concatenate([self._calmness_buffer, [calmness]], 0), 0, ALGO_WINDOW_TIME)
        if self._awareness_buffer is None:
            self._awareness_buffer = np.array([awareness])
        else:
            self._awareness_buffer = trim_data(np.concatenate([self._awareness_buffer, [awareness]], 0), 0, ALGO_WINDOW_TIME)
    ...
    def get_calmness_for_plot(self):
        return self._calmness_buffer if self._calmness_buffer is None else self._calmness_buffer.copy()

    def get_awareness_for_plot(self):
        return self._awareness_buffer if self._awareness_buffer is None else self._awareness_buffer.copy()
    ...
    def get_adhd_data_for_plot(self):
        return self._attention_buffer, self._eyemovement_buffer, self._stable_attention_buffer
...
class ZenLiteGUI:
    ...
    def on_plot_timer_timeout(self):
        if self.current_device_listener is not None:
            ...
            self.main_window.update_meditation_plot(self.current_device_listener.get_meditation_for_plot(),
                                                    self.current_device_listener.get_awareness_for_plot())
            ...
            self.main_window.update_adhd_plot(*self.current_device_listener.get_adhd_data_for_plot())
    ...
    def on_dev_pair_response(self, device, res):
        ...
        if res.success():
            ...
            self.current_device.dev_enable_ppg_smoothing(False, False)
