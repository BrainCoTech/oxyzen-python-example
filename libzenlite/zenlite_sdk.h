...
SDK_EXTERN DeviceOrientation zl_get_orientation(ZenLiteDevice* device);
...
typedef void (*ZLAttentionValueCB)(const char * device_id, float attention, float weighted_attention);
SDK_EXTERN int zl_set_attention_callback(ZenLiteDevice * device, ZLAttentionValueCB cb);
typedef void (*ZLValueCB)(const char * device_id, float value);
SDK_EXTERN int zl_set_stable_attention_callback(ZenLiteDevice * device, ZLValueCB cb);
SDK_EXTERN int zl_set_eye_movement_callback(ZenLiteDevice * device, ZLValueCB cb);
SDK_EXTERN int zl_set_stress_callback(ZenLiteDevice * device, ZLValueCB cb);
SDK_EXTERN int zl_set_relaxation_callback(ZenLiteDevice * device, ZLValueCB cb);
typedef void (*ZLMeditationValueCB)(const char * device_id, float meditation, float calmness, float awareness);
SDK_EXTERN int zl_set_meditation_callback(ZenLiteDevice * device, ZLMeditationValueCB cb);
...
// ZenLite msg
SDK_EXTERN int zl_write_data(ZenLiteDevice* device, char* data, int size, ConfigRespCB cb);
...
SDK_EXTERN void dev_enable_ppg_smoothing(ZenLiteDevice* device, bool heart_rate, bool spo2);
SDK_EXTERN void dev_set_tf_disabled(bool disabled);
//CFFI_DEF_END
