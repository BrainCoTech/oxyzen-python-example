...
def gen_msg_id():
    return libzenlite.zl_gen_msg_id()
...
class ZenLiteDeviceListener(abc.ABC):
    ...
    def on_attention(self, attention, weighted_attention):
        pass  # print("on_attention not implemented")

    def on_stable_attention(self, stable_attention):
        pass  # print("on_stable_attention not implemented")

    def on_eye_movement(self, eye_movement):
        pass  # print("on_eye_movement not implemented")
...
class ZenLiteDevice(ZenLiteDeviceListener):
    ...
    # ZenLite msg
    def zl_write_message(self, zen_lite_config, cb=None):
        if self.__uuid in ZenLiteDevice._device_pointer_map:
            msg_id = gen_msg_id()
            zen_lite_config.msg_id = msg_id
            serialized_data = zen_lite_config.SerializeToString()
            np_data = np.frombuffer(serialized_data, dtype=np.uint8)
            res = libzenlite.zl_write_data(ZenLiteDevice._device_pointer_map[self.__uuid], ffi.cast("char(*)", np_data.ctypes.data), len(np_data), ZenLiteDevice.__on_afe_config_response_internal)
            print("zl_write_message res:", res)
            if msg_id > 0:
                if cb is not None:
                    ZenLiteDevice._config_response_callbacks[msg_id] = cb
                return ZLError(ZLErrorCode.none)
            else:
                return ZLError(res)
        else:
            fatal_error("Calling zl_write_data before connecting to device")
    ...
    def dev_enable_ppg_smoothing(self, hear_rate, spo2):
        if self.__uuid in ZenLiteDevice._device_pointer_map:
            libzenlite.dev_enable_ppg_smoothing(ZenLiteDevice._device_pointer_map[self.__uuid], hear_rate, spo2)
        else:
            fatal_error("Calling dev_enable_ppg_smoothing before connecting to device")
        ...
        if isinstance(listener, ZenLiteDeviceListener):
            ...
            if self.__uuid in ZenLiteDevice._device_pointer_map:
                ...
                if listener is not None:
                    ...
                    libzenlite.zl_set_attention_callback(device_ptr, ZenLiteDevice.__on_attention_internal)
                    libzenlite.zl_set_stable_attention_callback(device_ptr, ZenLiteDevice.__on_stable_attention_internal)
                    libzenlite.zl_set_eye_movement_callback(device_ptr, ZenLiteDevice.__on_eye_movement_internal)
                    ...
                    libzenlite.zl_set_attention_callback(device_ptr, ffi.NULL)
                    libzenlite.zl_set_stable_attention_callback(device_ptr, ffi.NULL)
                    libzenlite.zl_set_eye_movement_callback(device_ptr, ffi.NULL)
    ...
    def create_device_with_scan_result(cls, result):
        ...
        name = ffi.string(result.name, 40).decode("utf-8")
        rssi = result.rssi
    ...
    @staticmethod
    @ffi.callback("void(char*, float, float, float)")
    def __on_meditation_internal(uuid_ptr, meditation, calmness, awareness):
        uuid = ffi.string(uuid_ptr, 40).decode("utf-8")
        if uuid in ZenLiteDevice._device_map:
            device = ZenLiteDevice._device_map[uuid]
            if device.__listener is not None:
                device.__listener.on_meditation(meditation, calmness, awareness)
        else:
            fatal_error("__meditation_internal:device unavailable for:" + uuid)

    @staticmethod
    @ffi.callback("void(char*, float, float)")
    def __on_attention_internal(uuid_ptr, attention, weighted_attention):
        uuid = ffi.string(uuid_ptr, 40).decode("utf-8")
        if uuid in ZenLiteDevice._device_map:
            device = ZenLiteDevice._device_map[uuid]
            if device.__listener is not None:
                device.__listener.on_attention(attention, weighted_attention)
        else:
            fatal_error("__attention_internal:device unavailable for:" + uuid)

    @staticmethod
    @ffi.callback("void(char*, float)")
    def __on_stable_attention_internal(uuid_ptr, stable_attention):
        uuid = ffi.string(uuid_ptr, 40).decode("utf-8")
        if uuid in ZenLiteDevice._device_map:
            device = ZenLiteDevice._device_map[uuid]
            if device.__listener is not None:
                device.__listener.on_stable_attention(stable_attention)
        else:
            fatal_error("__on_stable_attention_internal:device unavailable for:" + uuid)

    @staticmethod
    @ffi.callback("void(char*, float)")
    def __on_eye_movement_internal(uuid_ptr, eye_movement):
        uuid = ffi.string(uuid_ptr, 40).decode("utf-8")
        if uuid in ZenLiteDevice._device_map:
            device = ZenLiteDevice._device_map[uuid]
            if device.__listener is not None:
                device.__listener.on_eye_movement(eye_movement)
        else:
            fatal_error("__eye_movement_internal:device unavailable for:" + uuid)
    ...
    def __on_sleep_stage_internal(uuid_ptr, stage, conf, drowsiness):
        ...
        if uuid in ZenLiteDevice._device_map:
            ...
            fatal_error("__sleep_internal:device unavailable for:" + uuid)
...
class ZenLiteSDK:
    ...
    def set_log_level(cls, level):
        return libzenlite.zl_set_log_level(level)
